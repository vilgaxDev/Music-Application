<template>
  <div class="pages-wrapper">
    <v-card outlined>
      <v-card-title>
        <v-icon color="primary" x-large>mdi-text-box-multiple-outline</v-icon>
        <v-btn class="mx-2" dark small color="primary" @click="editPage('new')">
          <v-icon>mdi-plus</v-icon> {{ $t('New') }} {{$t('Page')}}
        </v-btn>
      </v-card-title>
      <v-data-table
...
export default {
  ...
  methods: {
    ...
    deletePage(page_id) {
      this.$confirm({
        message: `${this.$t("Are you sure you wanna delete this") + " " + this.$t('Page') + "?"}`,
        button: {
          no: this.$t('No'),
          yes: this.$t('Yes'),
        },
