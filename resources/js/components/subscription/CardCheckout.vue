<template>
  <v-card flat light>
    <v-card-text v-show="!isScriptLoading">
      ...
      >
        <div class="existing-card__details d-flex align-center">
          <div class="credit-card mr-2">
            ...
            ></v-img>
            <v-img
              v-if="$store.getters.getUser.card_brand == 'mastercard'"
            ...
            ></v-img>
            <v-img
              v-if="$store.getters.getUser.card_brand == 'discover'"
              src="/storage/defaults/images/billing/discover.png"
            ></v-img>
            <v-img
              v-if="$store.getters.getUser.card_brand == 'unionpay'"
              src="/storage/defaults/images/billing/unionpay.png"
            ></v-img>
          </div>
          <div class="existing-card__digits">
            <strong>****</strong>
            <strong>{{ $store.getters.getUser.card_last_four }}</strong>
          </div>
        </div>
      ...
      >
        <div class="loading-circle-button" v-if="isSubscriptionLoading">
          <loading-circle size="27" />
        </div>
        {{$t('Subscribe')}}
      </v-btn>
    </v-card-text>
    ...
    methods: {
        async subscribeWithStripe() {
            ...
            try {
                ...
                this.$emit('createSubscription', { gateway: 'stripe', paymentMethod})
            } catch (e) {
                if (!this.error) {
                    this.$emit('error', this.$t("Oops! Some error occurred. Try again later."))
