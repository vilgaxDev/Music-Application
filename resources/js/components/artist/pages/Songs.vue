<template>
    <div class="songs-wrapper">
        <v-card outlined>
            <v-card-title>
                <v-icon color="primary" x-large>mdi-music-note</v-icon>
                <v-btn
                    class="mx-2"
                    dark
                    small
        ...
        editSong(song) {
            if (song === "new") {
                this.songs.unshift({
                    ...
                    nb_plays: 0,
                    nb_likes: 0,
                    artist_id: this.$store.getters.getArtist.id,
                    artists: JSON.stringify([this.$store.getters.getArtist.displayname]),
                    public: true,
                    source: "",
                    source_format: "file"
                });
